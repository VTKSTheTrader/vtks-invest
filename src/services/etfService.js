...
const buildAccumulationPayload = (
  accumulation = {}
) => ({
  etf_id: Number(
    accumulation.etfId
  ...
  price: Number(
    accumulation.price || 0
  ),

  amount: Number(
    accumulation.amount || 0
  ),
...
export const getETFSummaries =
  async () => {
    const { data, error } =
      await supabase
        .from(
          "etf_portfolio_summary"
        )
        .select("*")
        .order("created_at", {
          ascending: false,
...
export const getETFSummaryById =
  async (id) => {
    if (!id) return null;
...
export const addETFAccumulation =
  async (accumulation) => {
    ...
    if (
      ...
      throw new Error(
        "ETF price must be greater than zero."
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Accumulation amount must be greater than zero."
      );
    }
    ...
    return normalizeAccumulation(
      data
    );
  };

export const updateETFAccumulation =
  async (
    id,
    accumulation
  ) => {
...
export const deleteETFAccumulation =
  async (id) => {
    if (!id) {
      throw new Error(
        "Accumulation ID is required."

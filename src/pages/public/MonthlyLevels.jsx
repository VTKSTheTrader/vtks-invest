...
export default function MonthlyLevels() {
  ...
  const loadLevels = async () => {
    try {
      ...
      const rows =
        await getPublishedMonthlyLevels();

      setLevels(rows || []);
    } catch (error) {
      console.error(
        "Public monthly levels load error:",
        error
      );
    ...
    <main className="public-monthly-levels-page">
      ...
      <section className="public-monthly-levels-section">
        ...
        {loading ? (
          ...
          <div className="public-monthly-levels-state">
            <h3>
              No published market levels available
            </h3>

            <p>
              Published public levels will appear
              here.
            </p>
          </div>
        ) : (
